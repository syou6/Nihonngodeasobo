...
> **⚠️ 2026-06 更新 — ピッチアクセント・ピボット後の現状**
>
> この計画書（Phase 1〜4）は完了済みの旧ロードマップ。現在の主軸は
> **日本語ピッチアクセント・トレーナー**（LP/価格/ファネルすべてピッチ訴求）。
>
> - トレーナー: 66語カリキュラム / ライブ＋実F0ネイティブ曲線 / 採点・コーチング /
>   アダプティブ復習・習得度 / クラウド同期 / セッションサマリー＋シェア画像
>   （`src/components/pitch/`, `src/lib/pitch-*`, `docs/APPLY_MIGRATIONS.md` 参照）
> - 価格: $8.99/mo + $59/yr（LP・アプリ・Stripe 整合）
> - Phase 4 相当: ユニット 420+ / E2E はピッチ・ファネルに整合済み
>   （auth系E2EはSupabase env必須 = CI/ステージングで実行）
> - 未完: `supabase/migrations/014_create_pitch_progress.sql` の本番適用
>   （適用までクラウド同期は無効のままローカルで動作）
